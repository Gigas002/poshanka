...
- [ ] One layer-shell surface per notification `id` from subscribe snapshots.
- [ ] SHM resize on configure; paint via Phase 2 `paint_card`.
- [ ] Diff `update` items → create / destroy / reposition surfaces; `[stack].gap` + `placement`.
- [ ] Wakeup pipe + `poll` loop; reload event re-reads poshanka config/theme.
